...
The pack has grown from 16 → 50 working nodes across Audio, Image, Latent,
Model Utilities, Prompt, and Workflow — every node that ships now actually
registers and runs. Prompt and Workflow were the two clearest gaps (wildcard
resolution and any-type graph control), and both are now covered. A few
gaps stand out as the next round of additions:

**1. A Latent ↔ Image bridge utility.** You have Latent Visualizer (latent →
...
**2. Batch/orchestration nodes**, since your production workflow (per your
...
or Quick LoRA Stacker configuration to JSON and back) would pair naturally
with Model Recipe Writer and let you version-control "known good"
merge/LoRA combinations the same way you already do for prompts.

**3. Video/animation support** is conspicuously absent given Latent
...
batch" to "get a shareable file" without leaving the pack. With Image
Save now in place, this is a natural extension: the same node family could
gain a "Save as GIF/animated WEBP" mode alongside Image Save's existing
PNG/JPEG/WEBP still-image output.

**4. A Model Recipe Writer ↔ Batch Folder Loader link.** Now that Batch
Folder Loader and Simple SDXL Loader both exist, a node that logs which
loader + checkpoint + VAE combination produced a given output (extending
your existing Model Recipe Writer concept) would close the loop between
"found a good checkpoint via batch scanning" and "reliably load that exact
setup again later."

**5. An Image ↔ Contact Sheet integration.** With Image Load/Save now
in the pack, a "Contact Sheet from Folder" node — using the same
`folder_paths`-scoped scanning pattern Batch Folder Loader established for
checkpoints — could tile every image in an output subfolder into a single
review grid, extending your existing Contact Sheet node's batch-in-memory
approach to batch-on-disk.

**6. Crop presets tied to social formats.** Image Resize & Crop's `fit`
and `fill` modes already handle arbitrary target dimensions, but a small
companion dropdown-driven node (Instagram post 1:1, Story 9:16, landscape
16:9, etc.) sitting on top of it would remove the "look up the exact pixel
dimensions" step from your existing Social Crop node's workflow, or could
simply be folded into Social Crop directly as new presets if you'd rather
not add a fourth Image resize-adjacent node to the category.
